 ...
 */
export function canUpdateEntryForDate(date: string, entryCreatedAt?: string): DateValidationResult {
  const today = getToday()
  const minDate = getMinAllowedDate()
 ...
 */
export function canDeleteEntry(entryDate: string): DateValidationResult {
  return {
    isValid: false,
...
export function getDateRestrictionMessage(): string {
  ...
  return `You can create or update log entries for today and the last 2 days (${minDate} to ${today}). Entries cannot be deleted.`
}
