  ...
  try {
    ...
    if (error instanceof Error) {
      if (error.message.includes('timeout')) {
        ...
        console.warn('Network error during authentication:', error.message);
...
export async function signUp(email: string, password: string) {
  try {
    ...
    const timeoutPromise = new Promise((_, reject) => 
      setTimeout(() => reject(new Error('Registration timeout - please check your internet connection and try again')), 15000)
    );
...
export async function resetPassword(email: string) {
  try {
    ...
    const timeoutPromise = new Promise((_, reject) => 
      setTimeout(() => reject(new Error('Password reset timeout - please check your internet connection and try again')), 15000)
    );
...
export async function updatePassword(password: string) {
  try {
    ...
    const timeoutPromise = new Promise((_, reject) => 
      setTimeout(() => reject(new Error('Password update timeout - please check your internet connection and try again')), 15000)
    );
...
export async function getCurrentUser(): Promise<User | null> {
  try {
    ...
    const { data: { user } } = await Promise.race([getUserPromise, timeoutPromise]) as Awaited<ReturnType<typeof supabase.auth.getUser>>;
    return user;
  } catch (error) {
    // Handle network errors gracefully
...
export async function createUserProfile(
  ...
  try {
    ...
    const timeoutPromise = new Promise((_, reject) => 
      setTimeout(() => reject(new Error('Profile creation timeout - please check your internet connection and try again')), 15000)
    );
    ...
    if (error instanceof Error) {
      if (error.message.includes('timeout')) {
        ...
        throw new Error('Connection timeout during profile creation - please check your internet connection and try again');
      } else if (error.message.includes('fetch failed') || error.message.includes('Failed to fetch')) {
        console.warn('Network error during profile creation:', error.message);
        throw new Error('Network connection failed during profile creation - please check your internet connection and try again');
      }
    }
