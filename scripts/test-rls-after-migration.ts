...
async function runScript() {
  ...
  try {
    ...
    const user = users.find((u: User) => u.email === USER_EMAIL)
    if (!user) {
    ...
    const { data: questionsRLS, error: questionsRLSError } = await supabase
      .from<RoleQuestion>("role_questions")
      .select("*")
    ...
    if (profileError) {
      ...
      console.log(`   - Is Admin: ${profile.role_id === ADMIN_ROLE_ID}`)
      console.log(`   - Is System Admin: ${profile.role_id === SYSTEM_ADMIN_ROLE_ID}`)
