-- Phase 3: Transaction-safe bulk update for scope_entry_kinds targets
-- Provides atomic default enforcement per target scope.
...
BEGIN
  ...
  IF v_active_count > 0 THEN
    -- Choose the default from payload among active; if multiple, first wins by sort_order then label.
    IF p_scope_type = 'profession_personal' THEN
    ...
    IF v_default_id IS NULL THEN
      -- If none marked default, pick first active
      IF p_scope_type = 'profession_personal' THEN
        SELECT id INTO v_default_id
        FROM public.scope_entry_kinds
        WHERE department_id = p_department_id
  ...
  -- Return active configs for target (sorted)
  IF p_scope_type = 'profession_personal' THEN
    RETURN QUERY
      SELECT *
      FROM public.scope_entry_kinds
