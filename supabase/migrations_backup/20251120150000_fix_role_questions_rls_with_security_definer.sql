-- Fix role_questions RLS using SECURITY DEFINER function
-- This bypasses RLS issues when checking user_profiles
...
STABLE  -- CRITICAL: Must be STABLE for use in RLS policies
AS $$
DECLARE
  user_role_id UUID;
  is_active_val BOOLEAN;
BEGIN
  -- Return false immediately if user_uuid is NULL
  ...
  -- Use COALESCE to handle NULL is_active values properly
  IF user_role_id IS NULL OR COALESCE(is_active_val, false) = false THEN
    RETURN false;
  END IF;
