  ...
  try {
    ...
    if (scopeType) {
      ...
      const normalizedConfigs = configs.map((config: any) => ({
        ...
        allowed_weekdays: Array.isArray(config.allowed_weekdays) ? config.allowed_weekdays : null,
        available_start_date: config.available_start_date || null,
        available_end_date: config.available_end_date || null,
      }))
