...
const ENTRY_KIND = "daily_supervisors_report"
...
export async function GET(request: Request) {
  ...
  const cacheKey = `supervisor-daily-reports-count:${user.id}:${marketingDepartmentId}:${window.key}:${visibilitySignature}`
  const cached = cache.get(cacheKey)
  if (cached && Date.now() - cached.at < CACHE_TTL_MS) return NextResponse.json({ kpi: "supervisor_daily_reports_count", value: cached.value, window: { ...window, hash: windowHash }, departmentId: marketingDepartmentId, meta: { aggregationMode: "live", aggregation: "count", aggregationLabel: "Supervisor daily reports in selected period", unit: "logs", cache: { hit: true } } })
  ...
  const promise = existing || (async () => {
    ...
    console.info("[marketing-kpi] ALLOW supervisor_daily_reports_count", { userId: user.id, departmentId: marketingDepartmentId, window: { key: window.key, hash: windowHash, preset: window.preset }, visibilitySignature, value: result.value })
    return result.value
  })()
  if (!existing) inflight.set(cacheKey, promise)
  try {
    const value = await promise
    return NextResponse.json({ kpi: "supervisor_daily_reports_count", value, window: { ...window, hash: windowHash }, departmentId: marketingDepartmentId, meta: { aggregationMode: "live", aggregation: "count", aggregationLabel: "Supervisor daily reports in selected period", unit: "logs", cache: { hit: false } } })
  } catch (err) {
