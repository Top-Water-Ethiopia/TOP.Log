...
import { MARKETING_FOLLOWERS_ADDED_KEYS } from "@/lib/marketing-kpis/followers-added"
...
export async function GET(request: Request) {
  ...
  const cacheKey = `followers-added:${user.id}:${marketingDepartmentId}:${window.key}:${visibilitySignature}`
  ...
  if (cached && Date.now() - cached.at < CACHE_TTL_MS) {
    return NextResponse.json({
      ...
      meta: { aggregationMode: "live", aggregation: "sum", aggregationLabel: "Total followers added in selected period", unit: "followers", cache: { hit: true } },
    })
  }
  ...
  const promise =
    ...
    (async () => {
      const pairs = await resolveConfiguredQuestionPairs({
        supabase,
        questionKeys: MARKETING_FOLLOWERS_ADDED_KEYS,
        expectedQuestionType: "number",
      })

      if (pairs.resolvedKeys.length === 0) {
        console.info("[marketing-kpi] followers_added missing_config", {
          userId: user.id,
          departmentId: marketingDepartmentId,
      ...
      console.info("[marketing-kpi] ALLOW followers_added", {
        userId: user.id,
        departmentId: marketingDepartmentId,
        window: { key: window.key, hash: windowHash, preset: window.preset },
        visibilitySignature,
  ...
  try {
    ...
    return NextResponse.json({
      ...
      window: { ...window, hash: windowHash },
      departmentId: marketingDepartmentId,
      meta: { aggregationMode: "live", aggregation: "sum", aggregationLabel: "Total followers added in selected period", unit: "followers", cache: { hit: false } },
    })
  } catch (err) {
