...
export async function GET(request: Request) {
  ...
  const cacheKey = `major-activities-count:${user.id}:${marketingDepartmentId}:${window.key}:${visibilitySignature}`
  const cached = cache.get(cacheKey)
  if (cached && Date.now() - cached.at < CACHE_TTL_MS) return NextResponse.json({ kpi: "major_activities_count", value: cached.value, window: { ...window, hash: windowHash }, departmentId: marketingDepartmentId, meta: { aggregationMode: "live", aggregation: "count", aggregationLabel: "Major activities in selected period", unit: "logs", cache: { hit: true } } })
  ...
  const promise = existing || (async () => {
    ...
    console.info("[marketing-kpi] ALLOW major_activities_count", { userId: user.id, departmentId: marketingDepartmentId, window: { key: window.key, hash: windowHash, preset: window.preset }, visibilitySignature, value: result.value })
    return result.value
  })()
  if (!existing) inflight.set(cacheKey, promise)
  try {
    const value = await promise
    return NextResponse.json({ kpi: "major_activities_count", value, window: { ...window, hash: windowHash }, departmentId: marketingDepartmentId, meta: { aggregationMode: "live", aggregation: "count", aggregationLabel: "Major activities in selected period", unit: "logs", cache: { hit: false } } })
  } catch (err) {
