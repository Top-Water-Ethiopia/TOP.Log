...
export async function GET(request: Request) {
  ...
  const cacheKey = `agents-contacted:${user.id}:${marketingDepartmentId}:${window.key}:${visibilitySignature}`
  const cached = cache.get(cacheKey)
  if (cached && Date.now() - cached.at < CACHE_TTL_MS) {
    return NextResponse.json({
      kpi: "agents_contacted",
      value: cached.value,
      ...
      meta: {
        ...
        aggregationLabel: "Distinct agents contacted in selected period",
        unit: "agents",
  ...
  const promise =
    ...
    (async () => {
      ...
      // Fetch all distinct subject_agent_id values for agent_contact entries in the window.
      // Supabase JS doesn't support COUNT(DISTINCT ...) directly, so we select the column
      // and deduplicate in JS.
      const { data, error } = await supabase
        .from("captain_log_entries")
        .select("subject_agent_id")
        .or(buildDepartmentCoalesceOrFilter(marketingDepartmentId))
        .eq("entry_kind", MARKETING_AGENT_CONTACTS_ENTRY_KIND)
        .gte("date", window.start)
        .lte("date", window.end)
        .not("subject_agent_id", "is", null)

      if (error) {
        console.info("[marketing-kpi] ERROR agents_contacted query_failed", {
          userId: user.id,
          departmentId: marketingDepartmentId,
      ...
      const uniqueAgentIds = new Set((data || []).map((row: any) => row.subject_agent_id).filter(Boolean))
      const value = uniqueAgentIds.size

      cache.set(cacheKey, { at: Date.now(), value })
      console.info("[marketing-kpi] ALLOW agents_contacted", {
        userId: user.id,
        departmentId: marketingDepartmentId,
        window: { key: window.key, hash: windowHash, preset: window.preset },
        visibilitySignature,
